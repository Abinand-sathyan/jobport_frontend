    ...
    if (token) {
      (async function invoke() {
        await recuiterdata(token).then((res) => {
          setRecruiterData(res.data.recruiterdata);
          setCompanyname(res.data.recruiterdata[0].company_name);
        });
      })();
    } else {
    ...
    try {
      if (true) {
        ...
        if (data.postlogo.name) {
          const date = Date.now(data.postlogo.name, "teck teck");
        ...
        if (result.success) {
          setisloading(false);
          message.success("Add job successfully!");
        } else {
          setisloading(false);
