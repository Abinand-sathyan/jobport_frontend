...
const Reqlogin = () => {
  ...
  const handleLogin = async (event) => {
    try {
      ...
      if (result.success) {
        localStorage.setItem("recruiterToken", response.data.recruiterToken);
        dispatch(
          recuiterLogin({
            recuiter: result.recruiterid,
            name: result.recruitername,
            token: result.recruiterToken,
          })
        );
                ...
                <div className="w-full p-8 ">
                  ...
                  <div className="mt-4">
                    <div className="flex justify-between">
                      ...
                      {/* <Link to="/forgetpassword"><p className="text-xs text-cyan-400">Forget Password?</p></Link> */}
                    </div>
                    <input
                      className=" text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full appearance-none"
                      type="password"
